...
RESTARTING_APP = "`The-TG-Bot is updating..\nThis can take a few minutes, please wait for atleast five minutes adn the try .alive'`"
INALID_HEROKU_API_KEY = "INVALID API KEY: To get a valid API key, goto https://dashboard.heroku.com/account \nOn the this website open API key tab.. \nThen click on **REVEAL** button and you will see your **HEROKU API key** \n**Copy and paste that in ENV variable `HEROKU_API_KEY`**"
INVALID_APP_NAME = "INVALID APP NAME: Please set the name of your bot in ENV variable `APP_NAME`"
# -- Constants End -- #
...
async def updater(message):
    ...
    if len(message_one) > 4095:
        ...
        # await bot.send_message(message.chat_id,"Changelog:\n"+message_one)
    ...
    if Config.HEROKU_API_KEY is not None:
        ...
        if len(heroku_applications) >= 1:
            logger.info("UPDATER: Heroku APP: "+Config.APP_NAME)
            heroku_app = None
            for i in heroku_applications:
                if i.name == Config.APP_NAME:
                    heroku_app = i
...
\nThen click on **REVEAL** button and you will see your **HEROKU API key**\
\nCopy and paste that in ENV variable `HEROKU_API_KEY`\
\n**If you have more than one heroku apps then setting `APP_NAME` ENV variable is also mandatory:**\
\nSimply copy and paste the bot app name in ENV variable APP_NAME\
"})
